...
function toggle(node) {
  ...
  if (childrenAreVisible) {
    ...
    if (lruByLevel[lv]) {
      lruByLevel[lv].remove(originalNode.id);
    }
  } else {
    // Show the children
...
function removeNodeAndDescendantsFromLRU(node, level) {
  ...
  if (node.children) {
    node.children.forEach(child => {
      removeNodeAndDescendantsFromLRU(child, level + 1);
    });
  }
}
